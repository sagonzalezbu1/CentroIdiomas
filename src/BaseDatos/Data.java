...
public class Data {
	...
	public static void cargarEstudiantes() {
		try {
			...
			while ((linea1 = br.readLine()) != null) {
				...
				Estudiante estudiante = new Estudiante(Long.parseLong(usuario[1]), usuario[0], usuario[2], usuario[3]); // Segundo constructor
				...
				for (String opcion : opciones) {
					estudiante.getMenu().anadirOpcion(Main.funcionalidades.get(opcion));
				}
			...
			while ((linea = br.readLine()) != null) {
				...
				if (cont % 2 == 0) {
					...
					for (String opcion : menu) {
						docente.getMenu().anadirOpcion(Main.funcionalidades.get(opcion));
					}
					cont++;
				}
			}
		...
		try {
			...
			while ((linea = br.readLine()) != null) {
				...
				if (cont % 2 == 0) {
					...
					for (String opcion : menu) {
						administrativo.getMenu().anadirOpcion(Main.funcionalidades.get(opcion));
					}
			...
			for (Estudiante estudiante : Archivo.getEstudiantes()) {
				...
				if (certificados.isEmpty()) {
					...
					for (Certificado certificado : certificados) {
						linea += certificado.getNombre() + ";" + certificado.getAlumno().getCedula() + ";" + certificado.getDocente().getCedula()+ ";" + certificado.getNota() + "-";
						}
				...
				linea = "";
				ArrayList<OpcionDeMenu> menu = estudiante.getMenu().getList();
				
				for (OpcionDeMenu opcion : menu) {
					linea += opcion.getNombre() + ";";
				}
	...
	/*escribirDocentes se encarga de escribir con formato correcto a los docentes en el archivo de texto docentes.txt para posteriormente ser leido por el metodo cargarDocentes. No recibe nada ni retorna nada.*/
	public static void escribirDocentes() {
		try {
			...
			for (Docente docente : Archivo.getDocentes()) {
				...
				String linea2 = "";
				ArrayList<OpcionDeMenu> menu = docente.getMenu().getList();
				
				for (OpcionDeMenu opcion : menu) {
					linea2 += opcion.getNombre() + ";";
				}
	...
	public static void escribirAdministrativos() {
		try {
			...
			for (Administrativo admnistrativo : Archivo.getAdministrativos()) {
				...
				pw.write(linea + "\n");
				String linea2 = "";
				ArrayList<OpcionDeMenu> menu = admnistrativo.getMenu().getList();
